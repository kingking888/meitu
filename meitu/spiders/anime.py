...
class AnimeSpider(scrapy.Spider):
    article_list = []
    name = 'anime'
    custom_settings = { "ITEM_PIPELINES": { "meitu.pipelines.AnimePipeline" : 300 } }
    start_urls = ['https://www.ilemiss.net/dongman/']

    def parse(self, response):
        for index in range(len(response.xpath('//div[@class="imainbox"]/ul/li')) + 1) :
            name = response.xpath('//div[@class="imainbox"]/ul/li[{}]//img/@alt'.format(index)).extract_first()
            ...
            if not name or not image_url or not link_url :
                continue
            article = BaseItem(image_name = name, image_url = image_url, link_url = link_url, dir = name)
